...
if __name__ == "__main__":
    ...
    # Run for California
    s = "California"
    fit_results = load_inference_result(us.states.lookup(s).fips)
    t0_simulation = datetime.fromisoformat(fit_results["t0_date"])

    a, b = _get_model_to_dataset_conversion_factors_for_state("California", t0_simulation)
    log.info(
        "Results for model to observed conversion:",
        state=s,
        hosp_rescaling_factor=a,
        icu_rescaling_factor=b,
    )
