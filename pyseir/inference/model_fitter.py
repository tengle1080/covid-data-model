...
class ModelFitter:
    ...
    def blend_prior(self):
        ...
        delta_x = x[1] - x[0]
        # This implements a hard lower limit of 0.98. TODO: As more data comes in, relax this.
        prior = gamma.pdf((x - 0.98) / .5, 1.3)
        likelihood = norm.pdf(x, 1.3, .3)
        posterior = prior * likelihood
        posterior = posterior / (posterior.sum() * delta_x)
        posterior_MAP_estimate = x[np.argmax(posterior)]
